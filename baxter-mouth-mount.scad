...
mounting_screw_d=3.5;     // M3 self tapping
...
mount=[
    ...
    servo_w + clearance + mounting_extra_len + wall
];
...
difference() {
    ...
    translate([-servo_len/2-clearance, 0, 0])
        union() {
            // Remove servo body
            translate([0, 0, wall]) cube([servo_len+clearance*2, mount[1], mount[2]]);
        };
    // Servo screw hole (left)
    translate([-(servo_len/2+servo_hole_offset), 0, servo_hole_h+wall])
